...
init -501 screen navigation():
    ...
    vbox:
        ...
        if not persistent.autoload or not main_menu:
            ...
            textbutton _("Load Game") action ShowMenu("load") sensitive (renpy.get_screen("load") == None)
...
init -1 style main_menu_version is main_menu_text:
    ...
    size 16
    outlines []
init -1 style main_menu_version_pom is main_menu_text:
    color "#fff"
    size 16
    outlines []
...
init -501 screen name_input(message):
    ...
    key "K_RETURN" action [Play("sound", gui.activate_sound), If(name_entry, true=Return(name_entry))]
    ...
    frame:
        ...
        input value VariableInputValue("name_entry", default=True, returnable=True) length 12 allow "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz" yalign 0.5

        hbox:
            xalign 0.5
            spacing 100

            textbutton _("OK") action If(name_entry, true=Return(name_entry))
